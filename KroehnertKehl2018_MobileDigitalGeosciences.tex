...
\begin{center}
		...
		\caption{Mean deviations between the smartphones Samsung Galaxy S8 and Google Nexus 5 for each orientation angle respectively captured by the three virtual sensors Android rotation vector, Improved Orientation Sensor 1 and 2. }
...
\begin{center}
		...
		\caption{Correlation between the orientation angles obtained by the smartphones Samsung Galaxy S8, Google Nexus 5 and Advanced Navigation Spatial \gls{IMU} with respect to the mentioned virtual sensors. }
...
Surprisingly, the results of all three virtual sensors of both smartphones show independently from each other almost the same behaviour as visualised in the figures \ref{fig:sensor_sensi:samsung_1st_a and fig:sensor_sensi:samsung_1st_a}. Especially \textit{Improved Orientation Sensor 1} and \textit{Android Rotation Vector} are very similar whereas \textit{Improved Orientation Sensor 2} seems to be slightly more stable as expected.
Beside this, it is easy to note that pitch and especially roll angles of both smartphones are close to the orientation of the reference \gls{IMU} even if the correlations in figure \ref{fig:sensor_sensi:imu_correlation_s8_nex5} show not much similarity caused by slight synchronisation errors visible in the graphs of figure \ref{fig:sensor_sensi:samsung_1st_b} which may be caused by averaging sensor values to establish comparability between both smartphones and \gls{IMU} respectively. 
However, large differences are visible for heading angle with errors up to 120 degrees even if there is no magnetic disturbance (Google Nexus 5, fixed IMU with running in).

Focussing on the rigid measurements fixed and fixed with added magnetic perturbations, sensor stability can be assessed by comparing their standard deviations summarised over full observation period of 2min 30sec (see figure \ref{fig:sensor_sensi:imu_sensor_stabi_s8_nex5}). Except heading angle, both devices show very stable measurements for all virtual sensors with standard deviations less than 0.1 degrees. Surprisingly, angles belonging to sensor \textit{Improved Orientation Sensor 2} show higher discrepancies although they should be less accurate but more stable. As expected, heading is most affected by non-rigidity especially when having magnetic sources close to the measuring devices, not only concerning smartphones. Also our professional \gls{IMU} shows issues due to heading stability mainly caused by drifting (see figure \ref{fig:sensor_sensi:samsung_1st_a}
